...
class CTAConfig:
    ...
    entry_decider_enabled: bool = True
    entry_decider_open_score: float = 0.55
    entry_decider_probe_score: float = 0.35
    entry_decider_watch_score: float = 0.20
    entry_decider_probe_size_ratio: float = 0.60
    entry_decider_use_family_adaptation: bool = True
    family_adaptation_min_closed_trades: int = 2
    family_adaptation_ttl_seconds: float = 600.0
    family_adaptation_boost_cap: float = 0.20
    family_adaptation_fast_start_trades: int = 3
    family_adaptation_fast_start_multiplier: float = 1.35
    loss_acceleration_decay_enabled: bool = True
    loss_acceleration_consecutive_losses: int = 3
    loss_acceleration_decay_score: float = 0.12
    loss_acceleration_decay_ttl_seconds: float = 3600.0
    pre_signal_observation_log_enabled: bool = True
    ...
    fast_track_min_entry_decider_score: float = 0.0
    fast_track_minimum_expected_rr: float = 0.0
    long_entry_location_score_min: float = -0.55
    short_entry_location_score_min: float = -0.45
    long_standard_min_confidence: float = 0.58
    short_standard_min_confidence: float = 0.55
    ...
    def __post_init__(self) -> None:
        ...
        self.entry_decider_enabled = bool(self.entry_decider_enabled)
        self.entry_decider_open_score = min(1.0, max(0.0, float(self.entry_decider_open_score)))
        self.entry_decider_probe_score = min(self.entry_decider_open_score, max(0.0, float(self.entry_decider_probe_score)))
        self.entry_decider_watch_score = min(self.entry_decider_probe_score, max(0.0, float(self.entry_decider_watch_score)))
        self.entry_decider_probe_size_ratio = min(1.0, max(0.1, float(self.entry_decider_probe_size_ratio)))
        self.entry_decider_use_family_adaptation = bool(self.entry_decider_use_family_adaptation)
        self.family_adaptation_min_closed_trades = max(1, int(self.family_adaptation_min_closed_trades))
        self.family_adaptation_ttl_seconds = max(30.0, float(self.family_adaptation_ttl_seconds))
        self.family_adaptation_boost_cap = max(0.0, float(self.family_adaptation_boost_cap))
        self.family_adaptation_fast_start_trades = max(1, int(self.family_adaptation_fast_start_trades))
        self.family_adaptation_fast_start_multiplier = max(0.5, float(self.family_adaptation_fast_start_multiplier))
        self.loss_acceleration_decay_enabled = bool(self.loss_acceleration_decay_enabled)
        self.loss_acceleration_consecutive_losses = max(1, int(self.loss_acceleration_consecutive_losses))
        self.loss_acceleration_decay_score = max(0.0, float(self.loss_acceleration_decay_score))
        self.loss_acceleration_decay_ttl_seconds = max(60.0, float(self.loss_acceleration_decay_ttl_seconds))
        self.pre_signal_observation_log_enabled = bool(self.pre_signal_observation_log_enabled)
        ...
        self.fast_track_min_entry_decider_score = min(1.0, max(0.0, float(self.fast_track_min_entry_decider_score)))
        self.fast_track_minimum_expected_rr = max(0.0, float(self.fast_track_minimum_expected_rr))
        self.long_entry_location_score_min = _normalize_signed_unit_interval(self.long_entry_location_score_min)
        self.short_entry_location_score_min = _normalize_signed_unit_interval(self.short_entry_location_score_min)
        self.long_standard_min_confidence = min(1.0, max(0.0, float(self.long_standard_min_confidence)))
        self.short_standard_min_confidence = min(1.0, max(0.0, float(self.short_standard_min_confidence)))
...
def load_config(config_path: str | Path) -> AppConfig:
    ...
    cta = CTAConfig(
        ...
        entry_decider_enabled=bool(cta_payload.get("entry_decider_enabled", True)),
        entry_decider_open_score=float(cta_payload.get("entry_decider_open_score", 0.55)),
        entry_decider_probe_score=float(cta_payload.get("entry_decider_probe_score", 0.35)),
        entry_decider_watch_score=float(cta_payload.get("entry_decider_watch_score", 0.20)),
        entry_decider_probe_size_ratio=float(cta_payload.get("entry_decider_probe_size_ratio", 0.60)),
        entry_decider_use_family_adaptation=bool(cta_payload.get("entry_decider_use_family_adaptation", True)),
        family_adaptation_min_closed_trades=int(cta_payload.get("family_adaptation_min_closed_trades", 2)),
        family_adaptation_ttl_seconds=float(cta_payload.get("family_adaptation_ttl_seconds", 600.0)),
        family_adaptation_boost_cap=float(cta_payload.get("family_adaptation_boost_cap", 0.20)),
        family_adaptation_fast_start_trades=int(cta_payload.get("family_adaptation_fast_start_trades", 3)),
        family_adaptation_fast_start_multiplier=float(cta_payload.get("family_adaptation_fast_start_multiplier", 1.35)),
        loss_acceleration_decay_enabled=bool(cta_payload.get("loss_acceleration_decay_enabled", True)),
        loss_acceleration_consecutive_losses=int(cta_payload.get("loss_acceleration_consecutive_losses", 3)),
        loss_acceleration_decay_score=float(cta_payload.get("loss_acceleration_decay_score", 0.12)),
        loss_acceleration_decay_ttl_seconds=float(cta_payload.get("loss_acceleration_decay_ttl_seconds", 3600.0)),
        pre_signal_observation_log_enabled=bool(cta_payload.get("pre_signal_observation_log_enabled", True)),
        ...
        fast_track_min_entry_decider_score=float(cta_payload.get("fast_track_min_entry_decider_score", 0.0)),
        fast_track_minimum_expected_rr=float(cta_payload.get("fast_track_minimum_expected_rr", 0.0)),
        long_entry_location_score_min=float(cta_payload.get("long_entry_location_score_min", -0.55)),
        short_entry_location_score_min=float(cta_payload.get("short_entry_location_score_min", -0.45)),
        long_standard_min_confidence=float(cta_payload.get("long_standard_min_confidence", 0.58)),
        short_standard_min_confidence=float(cta_payload.get("short_standard_min_confidence", 0.55)),
